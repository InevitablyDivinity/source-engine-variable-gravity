...
void CServerGameClients::GetPlayerLimits( int& minplayers, int& maxplayers, int &defaultMaxPlayers ) const
{
	minplayers = 1; 
	defaultMaxPlayers = 4;
	maxplayers = HORIZON_MAX_PLAYERS;
}
...
void CServerGameDLL::ApplyGameSettings( KeyValues *pKV )
{
	...
	const char *szMode = pKV->GetString( "game/mode", "horizon" );
	...
	if ( szGameMode && *szGameMode )
	{
		...
		r_mp_gamemode.SetValue( szGameMode );
	}


	if ( !Q_stricmp( szMode, "horizon" ) )
	{
		szBspName = pKV->GetString( "game/mission", NULL );
		if ( !szBspName )
		{
			Warning( "ApplyGameSettings: no map specified in dml mode!\n" );
			return;
		}
