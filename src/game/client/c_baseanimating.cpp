...
void C_BaseAnimating::GetPoseParameters( CStudioHdr *pStudioHdr, float poseParameter[MAXSTUDIOPOSEPARAM])
{
	...
	if (r_sequence_debug.GetInt() == entindex())
	{
		...
		for( i=0; i < pStudioHdr->GetNumPoseParameters(); i++)
		{
			...
			DevMsgRT( "%s %6.2f ", Pose.pszName(), poseParameter[i] * Pose.end + (1 - poseParameter[i]) * Pose.m_flGravityLerpStart );
		}
		DevMsgRT( "\n" );
	}
#endif
...
#if defined ( HL2MP ) || defined ( SDK_DLL ) || defined ( PORTAL2 ) // works for the modified CSS weapons included in the new template sdk.
