...
void GameSettings::Activate()
{
	...
	if ( m_drpGameType )
	{
		const char *szGameMode = m_pSettings->GetString( "game/mode", "horizon" );
		if ( szGameMode )
		{
			...
			else if ( !Q_stricmp( szGameMode, "horizon" ) )
			{
				m_drpGameType->SetCurrentSelection( "Horizon Level" );
			}	
		}
...
void GameSettings::OnCommand(const char *command)
{
	...
	else if( V_strcmp( command, "cmd_gametype_horizon" ) == 0 )
	{
		KeyValues *kvUpdate = new KeyValues( "update" );
		kvUpdate->SetString( "game/mode", "horizon" );
		KeyValues *kvModification = new KeyValues( "settings" );
		kvModification->AddSubKey( kvUpdate );
		UpdateSessionSettings( KeyValues::AutoDeleteInline( kvModification ) );
		UpdateSelectMissionButton();
...
#else
	const char *szGameType = m_pSettings->GetString( "game/mode", "horizon" );
	if ( !Q_stricmp( szGameType, "horizon" ) )
	{
		const char *szMission = m_pSettings->GetString( "game/mission", NULL );
		if ( szMission )
		{
...
void GameSettings::UpdateSelectMissionButton()
{
	...
	if ( m_pSettings && button )
	{
		...
		else if( !Q_stricmp( szGameType, "horizon" ) )
		{
			button->SetText( "Select Map" );
			button->SetHelpText( "Horizon select map example" );
		}
	}
	/*
