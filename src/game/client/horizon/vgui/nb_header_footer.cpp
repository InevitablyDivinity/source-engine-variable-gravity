...
CHorizon_Background_Movie *g_pBackgroundMovie = NULL;

CHorizon_Background_Movie* HorizonBackgroundMovie()
{
	if ( !g_pBackgroundMovie )
	{
		g_pBackgroundMovie = new CHorizon_Background_Movie();
	}
	return g_pBackgroundMovie;
}

CHorizon_Background_Movie::CHorizon_Background_Movie()
{
...
void CHorizon_Background_Movie::SetCurrentMovie( const char *szFilename )
{
...
void CHorizon_Background_Movie::ClearCurrentMovie()
{
#ifdef ASW_BINK_MOVIES
	if ( m_nBIKMaterial != BIKMATERIAL_INVALID )
	{
...
int CHorizon_Background_Movie::SetTextureMaterial()
{
#ifdef ASW_BINK_MOVIES
	if ( m_nBIKMaterial == BIKMATERIAL_INVALID )
		return -1;
...
void CHorizon_Background_Movie::Update()
{
	if ( engine->IsConnected() && HorizonGameRules() )
	{
...
void CNB_Header_Footer::PaintBackground()
{
	...
	if ( m_bMovieEnabled && HorizonBackgroundMovie() )
	{
		HorizonBackgroundMovie()->Update();
		if ( HorizonBackgroundMovie()->SetTextureMaterial() != -1 )
		{
