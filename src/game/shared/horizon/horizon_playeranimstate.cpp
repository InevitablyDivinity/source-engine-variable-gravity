...
ConVar horizon_facefronttime( "horizon_facefronttime", "0.0", FCVAR_REPLICATED, "How many seconds before marine faces front when standing still." );
ConVar horizon_max_body_yaw( "horizon_max_body_yaw", "60", FCVAR_REPLICATED, "Max angle body yaw can turn either side before feet snap." );
ConVar horizon_feetyawrate( "horizon_feetyawrate", "800",  FCVAR_REPLICATED, "How many degrees per second that we can turn our feet or upper body." );
...
//-----------------------------------------------------------------------------
CHorizonPlayerAnimState* CreateHorizonPlayerAnimState( CHorizonPlayer *pPlayer )
{
	...
	movementData.m_flMaxBodyYawDegrees = horizon_max_body_yaw.GetFloat();
	movementData.m_bUseAimSequences = false;

	// Create animation state for this player.
	CHorizonPlayerAnimState *pRet = new CHorizonPlayerAnimState();
	pRet->Init( pPlayer, movementData );

	// Specific Horizon player initialization.
	pRet->InitHorizonAnimState( pPlayer );
...
// Input  :  - 
//-----------------------------------------------------------------------------
CHorizonPlayerAnimState::CHorizonPlayerAnimState()
{
	m_pHorizonPlayer = NULL;
	m_bReloading = false;
	...
	// Don't initialize Horizon specific variables here. Init them in InitHorizonAnimState()
}
...
// Input  : *pPlayer - 
//-----------------------------------------------------------------------------
void CHorizonPlayerAnimState::InitHorizonAnimState( CHorizonPlayer *pPlayer )
{
	m_pHorizonPlayer = pPlayer;
}

void CHorizonPlayerAnimState::ComputeSequences( CStudioHdr *pStudioHdr )
{
...
void CHorizonPlayerAnimState::ClearAnimationLayers()
{
	if ( !m_pOuter )
		return;
...
void CHorizonPlayerAnimState::ClearAnimationState()
{
	m_bJumping = false;
	m_bFiring = false;
	m_bReloading = false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHorizonPlayerAnimState::DoAnimationEvent( PlayerAnimEvent_t event )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
Activity CHorizonPlayerAnimState::CalcMainActivity()
{
	if ( HandleJumping() )
	{
		return ACT_MP_JUMP;
	...
	else
	{
		...
		if ( m_pHorizonPlayer->GetFlags() & FL_DUCKING )
		{
...
// Output : Activity
//-----------------------------------------------------------------------------
Activity CHorizonPlayerAnimState::TranslateActivity( Activity actDesired )
{
	Activity translateActivity = BaseClass::TranslateActivity( actDesired );

	if ( GetHorizonPlayer()->GetActiveWeapon() )
	{
		translateActivity = GetHorizonPlayer()->GetActiveWeapon()->ActivityOverride( translateActivity, false );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CHorizonPlayerAnimState::ShouldResetMainSequence( int iCurrentSequence, int iNewSequence )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
float CHorizonPlayerAnimState::GetCurrentMaxGroundSpeed()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CHorizonPlayerAnimState::HandleJumping()
{
	if ( m_bJumping )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CHorizonPlayerAnimState::ShouldResetGroundSpeed( Activity oldActivity, Activity idealActivity )
{
...
int CHorizonPlayerAnimState::CalcFireLayerSequence(PlayerAnimEvent_t event)
{
	return SelectWeightedSequence( TranslateActivity(ACT_MP_ATTACK_STAND_PRIMARYFIRE) );
}

int CHorizonPlayerAnimState::CalcWeaponSwitchLayerSequence()
{
...
int CHorizonPlayerAnimState::CalcReloadLayerSequence()
{	
...
void CHorizonPlayerAnimState::ComputeWeaponSwitchSequence()
{
#ifdef CLIENT_DLL
...
int CHorizonPlayerAnimState::CalcReloadSucceedLayerSequence()
{	
...
int CHorizonPlayerAnimState::CalcReloadFailLayerSequence()
{	
	return GetOuter()->SelectWeightedSequenceFromModifiers( ACT_RELOAD_FAIL, m_ActivityModifiers.Base(), m_ActivityModifiers.Count() );
}

void CHorizonPlayerAnimState::UpdateLayerSequenceGeneric( int iLayer, bool &bEnabled,
...
void CHorizonPlayerAnimState::ComputeFireSequence()
{
#ifdef CLIENT_DLL
...
void CHorizonPlayerAnimState::ComputeReloadSequence()
{
#ifdef CLIENT_DLL
...
// Purpose: 
//-----------------------------------------------------------------------------
float CHorizonPlayerAnimState::SetOuterBodyYaw( float flValue )
{
...
// Input  : dt - 
//-----------------------------------------------------------------------------
void CHorizonPlayerAnimState::EstimateYaw()
{
	Vector est_velocity;
...
void CHorizonPlayerAnimState::ComputePoseParam_MoveYaw( CStudioHdr *pStudioHdr )
{
	int iMoveX = GetOuter()->LookupPoseParameter( pStudioHdr, "move_x" );
	int iMoveY = GetOuter()->LookupPoseParameter( pStudioHdr, "move_y" );
	if ( iMoveX < 0 || iMoveY < 0 )
...
void CHorizonPlayerAnimState::ComputePoseParam_BodyYaw()
{
	...
	else if ( (gpGlobals->curtime - m_flLastTurnTime) > horizon_facefronttime.GetFloat() )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHorizonPlayerAnimState::ComputePoseParam_BodyPitch(CStudioHdr *pStudioHdr)
{
...
float CHorizonPlayerAnimState::GetFeetYawRate( void )
{ 
	return horizon_feetyawrate.GetFloat();
}

void CHorizonPlayerAnimState::SetRenderAngles(QAngle angRenderAngles)
{
	m_angRender = angRenderAngles;
}